...
// v1.0.0, target SOVIET64.exe v1.1.1.7, TesmioLoader API 3
//
...
// SOVIET64.exe v1.1.1.7 / SHA-256
// ec05bb6257da31cfcaec639c8462683ee7bcf26158e0751a29a2d48025169522
static const usize RVA_CONSTRUCTION_UPDATE = 0x00159000u; // FUN_140159000
...
// Decode only ordinary position-independent MSVC prologue instructions. The
// hook fails closed on unknown layouts rather than relocating unsafe bytes.
static int DecodeSafePrologueInstruction(const u8* p, usize available,
                                         usize* length)
{
    ...
    if (op == 0x88 || op == 0x89 || op == 0x8A || op == 0x8B || op == 0x8D)
    {
        if (i >= available) return 0;
        const u8 modrm = p[i++];
        const u8 mod = (modrm >> 6) & 3;
        const u8 rm = modrm & 7;
        if (op == 0x8D && mod == 3) return 0;
        if (mod == 3) { *length = i; return 1; }
        if (mod != 1 && mod != 2) return 0; // refuse RIP-relative/no-disp forms
        if (rm == 4)
        {
            if (i >= available) return 0;
            ++i; // SIB remains register-relative for mod 1/2
        }
        const usize displacement = mod == 1 ? 1 : 4;
        if (i + displacement > available) return 0;
        *length = i + displacement;
        return 1;
    }
...
static int InstallVanillaHook(u8* target)
{
    ...
    {
        usize length = 0;
        if (!DecodeSafePrologueInstruction(target + stolen, 32 - stolen, &length) ||
            length == 0 || stolen + length > sizeof(expected))
        {
            if (H->log)
                H->log("freeheli  construction hook preflight refused at +0x%llX",
                       (u64)stolen);
            return 0;
        }
        stolen += length;
...
EXPORT int TsmPluginInit(const TsmHost* host, TsmPluginInfo* info)
{
    ...
    if (H->log)
        H->log("freeheli  v1.0.0 initialized: native construction completion path active; vehicle compatibility untouched");
    return 0;
}
